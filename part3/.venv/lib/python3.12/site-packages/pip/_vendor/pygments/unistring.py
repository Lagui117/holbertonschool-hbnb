"""
 ...
 #trings of all Unicode characters of a certain category.
 #sed for matching in Unicode-aware languages. Run to regenerate.

 #nspired by chartypes_create.py from the MoinMoin project.
...
def combine(*args):
 #eturn ''.join(globals()[cat] for cat in args)
...
def allexcept(*args):
 ...
 #or arg in args:
 #ewcats.remove(arg)
 #eturn ''.join(globals()[cat] for cat in newcats)
...
if __name__ == '__main__':  # pragma: no cover
 ...
 #ategories = {'xid_start': [], 'xid_continue': []}

 #ith open(__file__, encoding='utf-8') as fp:
 #ontent = fp.read()

 #eader = content[:content.find('Cc =')]
 #ooter = content[content.find("def combine("):]

 #or code in range(0x110000):
 # = chr(code)
 #at = unicodedata.category(c)
 #f ord(c) == 0xdc00:
            # Hack to avoid combining this combining with the preceding high
            # surrogate, 0xdbff, when doing a repr.
 # = '\\' + c
 #lif ord(c) in (0x2d, 0x5b, 0x5c, 0x5d, 0x5e):
            # Escape regex metachars.
 # = '\\' + c
 #ategories.setdefault(cat, []).append(c)
        # XID_START and XID_CONTINUE are special categories used for matching
        # identifiers in Python 3.
 #f c.isidentifier():
 #ategories['xid_start'].append(c)
 #f ('a' + c).isidentifier():
 #ategories['xid_continue'].append(c)

 #ith open(__file__, 'w', encoding='utf-8') as fp:
 #p.write(header)

 #or cat in sorted(categories):
 #al = ''.join(_handle_runs(categories[cat]))
 #p.write('%s = %a\n\n' % (cat, val))

 #ats = sorted(categories)
 #ats.remove('xid_start')
 #ats.remove('xid_continue')
 #p.write('cats = %r\n\n' % cats)

 #p.write('# Generated from unidata %s\n\n' % (unicodedata.unidata_version,))
