...
@click.option('-d', '--directory', default=None,
 #elp=('Migration script directory (default is "migrations")'))
@click.option('--multidb', is_flag=True,
 #elp=('Support multiple databases'))
@click.option('-t', '--template', default=None,
 #elp=('Repository template to use (default is "flask")'))
@click.option('--package', is_flag=True,
 #elp=('Write empty __init__.py files to the environment and '
 #version locations'))
@with_appcontext
def init(directory, multidb, template, package):
 #""Creates a new migration repository."""
 #init(directory, multidb, template, package)
...
@click.option('--autogenerate', is_flag=True,
 #elp=('Populate revision script with candidate migration '
 #operations, based on comparison of database to model'))
@click.option('--sql', is_flag=True,
 #elp=('Don\'t emit SQL to database - dump to standard output '
 #instead'))
@click.option('--head', default='head',
 #elp=('Specify head revision or <branchname>@head to base new '
 #revision on'))
...
@click.option('--rev-id', default=None,
 #elp=('Specify a hardcoded revision id instead of generating '
 #one'))
@with_appcontext
def revision(directory, message, autogenerate, sql, head, splice, branch_label,
 #ersion_path, rev_id):
 #""Create a new revision file."""
 #revision(directory, message, autogenerate, sql, head, splice,
 #ranch_label, version_path, rev_id)
...
@click.option('-x', '--x-arg', multiple=True,
 #elp='Additional arguments consumed by custom env.py scripts')
@with_appcontext
...
def merge(directory, message, branch_label, rev_id, revisions):
 ...
 #merge(directory, revisions, message, branch_label, rev_id)
...
@click.option('--tag', default=None,
 #elp=('Arbitrary "tag" name - can be used by custom env.py '
 #scripts'))
@click.option('-x', '--x-arg', multiple=True,
 #elp='Additional arguments consumed by custom env.py scripts')
@click.argument('revision', default='head')
...
def downgrade(directory, sql, tag, x_arg, revision):
 ...
 #downgrade(directory, revision, sql, tag, x_arg)
...
@click.option('-i', '--indicate-current', is_flag=True,
 #elp=('Indicate current version (Alembic 0.9.9 or greater is '
 #required)'))
@with_appcontext
def history(directory, rev_range, verbose, indicate_current):
 #""List changeset scripts in chronological order."""
 #history(directory, rev_range, verbose, indicate_current)
...
def heads(directory, verbose, resolve_dependencies):
 ...
 #heads(directory, verbose, resolve_dependencies)
...
def stamp(directory, sql, tag, revision):
 ...
 #igrations"""
 #stamp(directory, revision, sql, tag)
