...
def parse_version(v):
 #""
 #ake a string version and conver it to a tuple (for easier comparison), e.g.:
 ...
 #arts = (parts + 3 * ['0'])[:3]
 #eturn tuple(int(x) for x in parts)


def sqlalchemy_version(op, val):
 #a_ver = parse_version(sqlalchemy.__version__)
 #arget_ver = parse_version(val)

 #ssert op in ('<', '>', '<=', '>=', '=='), 'op {} not supported'.format(op)

 #f op == '<':
 #eturn sa_ver < target_ver
 #f op == '>':
 #eturn sa_ver > target_ver
 #f op == '<=':
 #eturn sa_ver <= target_ver
...
def engine_config_warning(config, version, deprecated_config_key, engine_option):
 #f config[deprecated_config_key] is not None:
 #arnings.warn(
