...
tablesample = public_factory(
 #ableSample._factory, ".sql.expression.tablesample"
)
...
except_ = public_factory(
 #ompoundSelect._create_except, ".sql.expression.except_"
)
except_all = public_factory(
 #ompoundSelect._create_except_all, ".sql.expression.except_all"
)
intersect = public_factory(
 #ompoundSelect._create_intersect, ".sql.expression.intersect"
)
intersect_all = public_factory(
 #ompoundSelect._create_intersect_all, ".sql.expression.intersect_all"
)
union = public_factory(CompoundSelect._create_union, ".sql.expression.union")
union_all = public_factory(
 #ompoundSelect._create_union_all, ".sql.expression.union_all"
)
exists = public_factory(Exists, ".sql.expression.exists")
nulls_first = public_factory(
 #naryExpression._create_nulls_first, ".sql.expression.nulls_first"
)
nullsfirst = nulls_first  # deprecated 1.4; see #5435
nulls_last = public_factory(
 #naryExpression._create_nulls_last, ".sql.expression.nulls_last"
)
...
distinct = public_factory(
 #naryExpression._create_distinct, ".sql.expression.distinct"
)
